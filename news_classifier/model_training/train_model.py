...
class NewsPipeline:
    ...
    def train_and_select(self, data):
        ...
        # ── DEPLOYMENT FIX ────────────────────────────────────────
        # KNN stores a full copy of X_train (2000+ rows × 3000 cols of
        # float64 = ~50 MB). We always re-train Naive Bayes on the full
        # dataset for the saved model: it is tiny (<1 MB) and almost as
        # accurate. The benchmark above is still shown for academic
        # comparison — the best model name is printed correctly.
        print(f"\n[4/4] Re-training Naive Bayes on full dataset for slim pkl...")
        slim_clf = NaiveBayesClassifier(alpha=0.5)
        slim_clf.fit(X, list(labels))
        slim_clf.name = f"Naive Bayes (slim; benchmark winner: {best_clf.name})"
        self.classifier = slim_clf
        print(f"    Pipeline ready  |  Saved classifier: Naive Bayes (slim pkl)")
        return self
...
if __name__ == "__main__":
    ...
    print("\n" + "="*65)
    print("  SAMPLE PREDICTIONS  (using saved slim model)")
    print("="*65)
    ...
    print("\n" + "="*65)
    print("  Done! Pkl is now deployment-safe (under 5 MB).")
    print("="*65)
